...
class ApiSqsLambdaStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        # cdk2d2 connections
        queue.node.add_metadata("Show","true")
        base_api.node.add_metadata("Show","true")
        sqs_lambda.node.add_metadata("Show","true")

        base_api.node.add_metadata("Connection", sqs_lambda.node.id)
        sqs_lambda.node.add_metadata("Connection",queue.node.id)
